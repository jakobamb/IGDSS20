...
public class MouseManager : MonoBehaviour
{
    ...
    void Zooming(){
        if(Input.GetAxis("Mouse ScrollWheel") < 0f){
            if(playerCam.transform.position[1]<120){
                playerCam.transform.Translate(0,0,Input.GetAxis("Mouse ScrollWheel") * scrollSensitivity);
            }
        }
        else if(Input.GetAxis("Mouse ScrollWheel") > 0f){
            if(playerCam.transform.position[1]>20){
                playerCam.transform.Translate(0,0,Input.GetAxis("Mouse ScrollWheel") * scrollSensitivity);
            }
        }
    }
    ...
    void Panning(){
        ...
        if (Input.GetMouseButton(1))
        {
            ...
            Vector3 newPosition = playerCam.transform.position + translateVector;
            ...
            {
                // check for limits
                playerCam.transform.Translate(delta.y * panSensitivity, 0, -delta.x * panSensitivity, Space.World);
            }
